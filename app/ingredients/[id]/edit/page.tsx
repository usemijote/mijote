...
export default async function EditIngredientPage({
  ...
  const { data: ingredient } = await supabase
    .from('ingredients')
    .select('id, nom, categorie_id, date_achat, date_peremption, quantite, unite')
    .eq('id', id)
    .single<IngredientDetail>()
        ...
        <form action={updateIngredient} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              type="date"
              required
              defaultValue={ingredient.date_achat}
              className="w-full px-4 py-2.5 bg-zinc-900 border border-zinc-800 text-zinc-50 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-500"
            />
          </div>

          <div>
            <label htmlFor="date_peremption" className="block text-sm font-medium text-zinc-300 mb-2">
              Date de péremption
            </label>
            <input
              id="date_peremption"
              ...
              type="date"
              required
              defaultValue={ingredient.date_peremption}
              className="w-full px-4 py-2.5 bg-zinc-900 border border-zinc-800 text-zinc-50 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-500"
            />
          </div>
